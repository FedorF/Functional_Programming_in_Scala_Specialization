...
object Anagrams extends App {
  ...
  def wordOccurrences(w: Word): Occurrences = {
    w.toLowerCase.toList.groupBy(x => x).map(t => (t._1, t._2.length)).toList.sortWith((x, y) => x._1 < y._1)
  }

  /** Converts a sentence into its character occurrence list. */
  def sentenceOccurrences(s: Sentence): Occurrences = wordOccurrences(s.mkString(""))

  // val s = List("Cat", "Sit", "On", "Floor")
  ...
  //  println(dictionaryByOccurrences.get(wordOccurrences("grab")))
  ...
  def combinations(occurrences: Occurrences): List[Occurrences] = {
    ...
    def hasUniqueChars(xs: Occurrences): Boolean = {
      val cs = for {
        pair <- xs
      } yield pair._1
      cs.length == cs.distinct.length
    }


    {
      (0 to basis.length) flatMap basis.combinations
    }.toList.filter(hasUniqueChars)

  }

  //  println("\n")
  //  val xs = List(('a', 2), ('b', 2), ('h', 5))

  //println(combinations(xs))
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences = {
    x.map(el => (el._1, el._2 - y.toMap.getOrElse(el._1, 0))).filter(_._2 > 0).sortWith((x, y) => x._1 < y._1)
  }

  //  println(subtract(wordOccurrences("GUUUgle"), wordOccurrences("gogle"))) // --> (o, 1)
